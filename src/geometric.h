...
auto Transform(const std::vector<S>& src, F f) -> std::vector<decltype(f(S()))>
{
    std::vector<decltype(f(S()))> dst(src.size());
...
std::vector<T*> Addresses(std::vector<T>& a)
{
    ...
    for (auto& e : a)
        p.push_back(&e);
    return p;
} // not sure what was wrong with { Transform(a, [](T &t)->T*{return &t; }); }
...
inline float4x4 MatrixFromVfovAspect(float vfov, float aspect, float n, float f)
{
...
inline float4x4 MatrixFromLookVector(const float3& fwd, const float3& up)
{
    ...
    return { { s.x, u.x, -f.x, 0 }, { s.y, u.y, -f.y, 0 }, { s.z, u.z, -f.z, 0 }, { 0, 0, 0, 1 } };
}
...
struct Pose // Value type representing a rigid transformation consisting of a translation and rotation component
{
    ...
    Pose(const float3& p, const float4& q)
        : position(p)
        , orientation(q)
    {
    }
...
inline float3x3 Inertia(const float3* vertices, const int3* tris, const int count, const float3& com)
{
    ...
    for (int i = 0; i < count; i++) // for each triangle
    {
        ...
        for (int j = 0; j < 3; j++) {
            int j1 = (j + 1) % 3;
            int j2 = (j + 2) % 3;
            diag[j] += (A[0][j] * A[1][j] + A[1][j] * A[2][j] + A[2][j] * A[0][j] + A[0][j] * A[0][j] + A[1][j] * A[1][j] + A[2][j] * A[2][j]) * d; // divide by 60.0f later;
            offd[j] += (A[0][j1] * A[1][j2] + A[1][j1] * A[2][j2] + A[2][j1] * A[0][j2] + A[0][j1] * A[2][j2] + A[1][j1] * A[0][j2] + A[2][j1] * A[1][j2] + A[0][j1] * A[0][j2] * 2 + A[1][j1] * A[1][j2] * 2 + A[2][j1] * A[2][j2] * 2) * d; // divide by 120.0f later
        }
    }
...
{
    plane.xyz() = cdiv(plane.xyz(), scaling);
    plane /= magnitude(plane.xyz());
}

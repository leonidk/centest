...
sgbmMatch::sgbmMatch(int w, int h, int d, int m)
    : StereoMatch(w, h, d, m), costs(w*d), costsSummed(w*d),
    censusLeft(w*h, 0), censusRight(w*h, 0)
{
}
       ...
       if(USE_SGM){
        ...
        for (int x = B_R; x < width - B_R; x++) {
            ...
            auto spL = (minLIdx > 0 && minLIdx < maxdisp-1) ? 
                subpixel((float)nL, (float)nC, (float)nR) : 0;
            // subpixel right
            ...
            auto spR = (minLIdx > 0 && minLIdx < maxdisp - 1) ?
                subpixel((float)rL, (float)rC, (float)rR) : 0;
